export type ResponseActionWordExportOptions = {
  label: string;
  tooltip: string;
...
export const WORD_EXPORT_BUTTON_TEST_ID = 'gv-export-word-button';
const WORD_EXPORT_ICON_NAME = 'description';
...
type BoundWordExportButton = HTMLElement & {
  __gvWordExportHandler?: (event: Event) => void;
};
...
function updateButtonLabelAndTooltip(
  ...
  button.setAttribute('data-gv-word-export-label', label);
...
function updateButtonIcon(button: HTMLElement): void {
  ...
  if (icon.hasAttribute('fonticon')) {
    icon.setAttribute('fonticon', WORD_EXPORT_ICON_NAME);
  }
  icon.textContent = WORD_EXPORT_ICON_NAME;
}
...
function bindButtonClick(
  ...
  const typed = button as BoundWordExportButton;
  if (typed.__gvWordExportHandler) {
    interactive.removeEventListener('click', typed.__gvWordExportHandler);
  }
  ...
  typed.__gvWordExportHandler = handler;
...
function ensureButtonPosition(
  ...
  wordExportButton: HTMLElement,
  moreButton: HTMLElement,
): void {
  if (!moreButton.parentElement) {
    if (!container.contains(wordExportButton)) container.appendChild(wordExportButton);
    return;
  }

  if (wordExportButton !== moreButton.previousElementSibling) {
    moreButton.parentElement.insertBefore(wordExportButton, moreButton);
  }
}

function injectIntoActionContainer(
  container: HTMLElement,
  options: ResponseActionWordExportOptions,
): HTMLElement | null {
  ...
  const existing = findButtonByTestId(container, WORD_EXPORT_BUTTON_TEST_ID);
  if (existing) {
  ...
  cloned.setAttribute('data-test-id', WORD_EXPORT_BUTTON_TEST_ID);
...
export function injectResponseActionWordButtons(
  root: ParentNode,
  options: ResponseActionWordExportOptions,
): HTMLElement[] {
