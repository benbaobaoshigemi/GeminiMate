...
describe('FormulaCopyService', () => {
  ...
  class TestClipboardItem {
    public static supports = vi.fn().mockReturnValue(true);
  ...
  beforeEach(() => {
    ...
    TestClipboardItem.supports.mockReturnValue(true);
  ...
  it('should generate MathML when format is unicodemath (now mapped to MathML)', async () => {
    ...
    expect(clipboardItem.dataByType['text/plain']).toBeUndefined();
  ...
  it('should prefer rendered DOM MathML when available', async () => {
    vi.mocked(temml.renderToString).mockReturnValue(
      '<math xmlns="http://www.w3.org/1998/Math/MathML"><mrow><mi>fallback</mi></mrow></math>',
    );
    ...
    expect(temml.renderToString).not.toHaveBeenCalled();
    expect(writeMock).toHaveBeenCalled();
  ...
  it('should retry with html-only clipboard when MathML MIME is rejected', async () => {
    vi.mocked(temml.renderToString).mockReturnValue(
    ...
    expect(firstClipboardItem.dataByType['text/plain']).toBeUndefined();

    const secondItemsUnknown = writeMock.mock.calls[1]?.[0] as unknown;
    const secondItems = secondItemsUnknown as TestClipboardItem[];
    const secondClipboardItem = secondItems[0];
    expect(secondClipboardItem.dataByType['text/html']).toBeDefined();
    expect(secondClipboardItem.dataByType['application/mathml+xml']).toBeUndefined();
    expect(secondClipboardItem.dataByType['text/plain']).toBeUndefined();
