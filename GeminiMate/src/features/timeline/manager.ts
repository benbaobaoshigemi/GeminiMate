...
export class TimelineManager {
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      if (g.chrome?.storage?.local || g.browser?.storage?.local) {
        res = await new Promise((resolve) => {
          const storage = g.chrome?.storage?.local || g.browser?.storage?.local;
          storage.get(
            defaults as Record<string, unknown>,
            (items: Record<string, unknown>) => {
              if (g.chrome?.runtime?.lastError) {
                console.error(
                  `[Timeline] chrome.storage.local.get failed: ${g.chrome.runtime.lastError.message}`,
                );
                resolve(null);
              } else {
                resolve(items);
        ...
        if (onChanged) {
          onChanged.addListener((changes: Record<string, { newValue: unknown }>, area: string) => {
            ...
            if (changes?.geminimate_timeline_enabled) {
              this.timelineEnabled = !!changes.geminimate_timeline_enabled.newValue;
              this.applyTimelineVisibility();
            }
  ...
  private applyTimelineThickness(): void {
    if (!this.ui.timelineBar) return;
    // Whole UI width change as requested
    this.ui.timelineBar.style.width = `${this.timelineWidth}px`;
    const dotSize = Math.max(4, Math.min(14, Math.floor(this.timelineWidth / 2)));
    // Adjust track and element sizes relative to total width
    this.ui.timelineBar.style.setProperty('--timeline-dot-size', `${dotSize}px`);
  ...
  private savePosition(): void {
    ...
    if (g.chrome?.storage?.local?.set) {
      g.chrome.storage.local.set({ geminiTimelinePosition: position });
    } else if (g.browser?.storage?.local?.set) {
      g.browser.storage.local.set({ geminiTimelinePosition: position });
    }
  }
  ...
  private async initKeyboardShortcuts(): Promise<void> {
    try {
      ...
      this.shortcutUnsubscribe = keyboardShortcutService.subscribe((action: string) => {
        if (action === 'timeline:previous') {
          this.enqueueNavigation('previous', false);
        } else if (action === 'timeline:next') {
          this.enqueueNavigation('next', false);
        }
      });
    } catch (error) {
